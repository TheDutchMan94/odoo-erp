...
class Meeting(models.Model):
    ...
    def to_extend_sprint(self):
        ...
        for meeting in self:
            if meeting.duration:
                ...
                vals['stop_datetime'] = datetime.strftime(
                    stop_date, DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def validate(self, meeting):
        ...
        user_id = meeting.user_id.id or self.env.user
        emp_id = self.env.user.employee_ids[0]
...
class ProjectScrumSprint(models.Model):
        ...
        for record in self:
            ...
            if not stories:
                ...
                self.message_post(body=_(
                    "The sprint '%s' has been "
                    "Opened." % record.sprint_number),
    ...
    def button_close(self):
        for sprint in self:
            ...
            for meeting in sprint.meeting_ids:
                meeting.write({'status': 'performed'})
    ...
    def _compute_effective_velocity(self):
        ...
        for sprint in self:
            velocity = \
                sum(backlog.complexity for backlog in sprint.backlog_ids)
...
class ScrumMeeting(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get('start_datetime') and vals.get('duration'):
            stop_date = datetime.strptime(vals.get('start_datetime'),
                                          DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(
                hours=duration)
            vals['stop_datetime'] = datetime.strftime(stop_date,
                                                      DEFAULT_SERVER_DATETIME_FORMAT)
        if vals.get('start'):
            stop_date = datetime.strptime(vals.get('start'),
                                          DEFAULT_SERVER_DATETIME_FORMAT) + timedelta(
                hours=duration)
            vals['stop_datetime'] = datetime.strftime(stop_date,
                                                      DEFAULT_SERVER_DATETIME_FORMAT)
    ...
    def send_email(self):
        ...
        if context.get('type') == 'owner':
            ...
            if not self.sprint_id.scrum_master_id:
                raise ValidationError(_(
                    'Please select scrum master in %s') % self.sprint_id.name)
            if self.sprint_id.scrum_master_id.partner_id and self.sprint_id. \
                ...
                if res is False:
                    raise ValidationError(_(
                        'Email could not be sent to the scrum master %s')
                                          % self.sprint_id.
        ...
        if context.get('type') == 'master':
            if not self.sprint_id:
                raise ValidationError(_(
                    'Please select Sprint'))
            if not self.sprint_id.product_owner_id:
                raise ValidationError(_(
                    'Please select product owner in %s') % self.sprint_id.name)
            if self.sprint_id.product_owner_id.partner_id and self.sprint_id. \
                ...
                raise ValidationError(_(
                    'Please provide email for product owner'
                    ' defined on sprint.'))
    ...
    def email_send(self, partner_id):
        ...
        vals = {}
        mail_temp = self.env.ref('project_scrum_agile.email_template_project_scrum')
        body = """
                        <div>
                        Hello %s,
                        <br/>
    ...
    def check_partners_email(self, partner_ids):
        ...
        """
        partner_email_lst = \
            [partner for partner in partner_ids if not partner.email]
    ...
    @api.onchange('start_datetime', 'allday', 'stop_date')
    def onchange_dates(self):
        ...
        elif self.start_datetime and self.allday and not self.start_date:
            self.duration = 24.00
            self.start = start
            self.stop = start + timedelta(hours=self.duration)
            self.start_date = start.date()
        elif self.start_datetime and self.allday and not self.stop_date:
            self.duration = 24.00
            self.start = start
            self.stop = start + timedelta(hours=self.duration)
            self.start_date = start.date()
            self.stop_date = self.stop.date()

        if self.stop_date:
            self.duration = self._get_duration(self.start_date, self.stop_date)
...
class ProjectScrumProductBacklog(models.Model):
    ...
    open = fields.Boolean('Open', track_visibility='onchange', default=True)
