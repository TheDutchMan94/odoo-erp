    ...
    def my_meetings(self, **post):
        ...
        for meeting in meetings:
            if not meeting.allday:
                if dict.get(meeting.start_datetime and fields.Date.to_string(meeting.start_datetime)[:10], False):
                    dict[meeting.start_datetime[:10]].append(meeting.id)
                else:
                    dict.update({meeting.start_datetime and fields.Date.to_string(meeting.start_datetime)[:10]: [meeting.id]})
            else:
...
#         result = {'estimated_hours': sprint_list1, 'spent_hours': sprint_list2}
    ...
    def get_meeting_data(self, **post):
        ...
        for scrum in meetings:
            start = datetime.strptime(scrum.start_datetime,
                                      "%Y-%m-%d %H:%M:%S")
